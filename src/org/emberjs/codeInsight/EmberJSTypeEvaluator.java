 ...
 */
public class EmberJSTypeEvaluator extends JSTypeEvaluator {
    public EmberJSTypeEvaluator(BaseJSSymbolProcessor.EvaluateContext context,
    ...
    protected boolean addTypeFromElementResolveResult(JSReferenceExpression expression,
        ...
        if (resolveResult instanceof JSDefinitionExpression) {
            ...
            if (resolveParent instanceof EmberJSAsExpression) {
                ...
                return true;
            } else if (resolveParent instanceof EmberJSRepeatExpression) {
                if (calculateRepeatParameterType((EmberJSRepeatExpression)resolveParent)) {
                    return true;
                }
            }
        }
        if (resolveResult instanceof JSParameter &&
                EmberIndexUtil.hasEmberJS(resolveResult.getProject())) {
    ...
    // does Ember have any equivalent of this?
    private boolean calculateRepeatParameterType(EmberJSRepeatExpression resolveParent) {
    ...
    private static PsiElement findReferenceExpression(EmberJSRepeatExpression parent) {
